...
// その1件をTC5のどちらに入れるか。
// 古い記録には区分が入っていないので、そのときは登録してある人の区分を使う
function leaveTc5Mode(leave, emp) {
  return (leave && leave.mode) || (emp && emp.mode) || 'normal';
}
...
let inTc5Mode = 'normal';              // その有給をTC5の「通常」と「スイミング」どちらに入れるか
...
function applyInputMode(fill) {
  ...
  // 正職はTC5を使わないので、振り分けの選択も出さない
  document.getElementById('inTc5ModeField').classList.toggle('hidden', !emp || !!staff);
  if (fill) setTc5Mode((emp && emp.mode) || 'normal');
  else setTc5Mode(inTc5Mode);
  applyShiftDefault();
  if (staff && inputType !== 'hours') setInputType('hours', fill);
  else setInputType(inputType, fill);
}

// TC5の「通常」「スイミング」どちらに入れるかを選ぶ
function setTc5Mode(mode) {
  inTc5Mode = TC5_MODES[mode] ? mode : 'normal';
  document.querySelectorAll('#inTc5Mode button').forEach(b =>
    b.classList.toggle('active', b.dataset.tc5 === inTc5Mode));
  const emp = getEmp(document.getElementById('inEmp').value);
...
function renderLeaveTable() {
  ...
  tbody.innerHTML = rows.slice(0, 500).map(({ l, emp }) => {
    ...
    return `<tr${editing ? ' class="editing"' : ''}>
      ...
      <td>${esc(emp.name)}${(part ? leaveTc5Mode(l, emp) === 'swim' : emp.mode === 'swim')
        ? ' <span class="tag swim">SW</span>' : ''}</td>
      <td>${content}</td>
      <td>${time}</td>
      <td>
...
function buildTc5Json(from, to, mode) {
  ...
  for (const l of DB.leaves) {
    ...
    const emp = getEmp(l.empId);
    if (!emp || leaveTc5Mode(l, emp) !== mode) continue;
